...
| T04 | Create Supabase `pets` table (schema per SPEC.md §Data Models — includes `current_activity`, `current_scene`, `last_activity_at`) | Person A | TODO |
| T05 | Configure Supabase Storage bucket for pet photos and pixel art | Person A | TODO |
| T06 | Enable Google OAuth in Supabase Auth dashboard | Person A | TODO |
| T07 | Deploy skeleton to Vercel + confirm Vercel → Supabase connection works (`GET /api/health`) | Person A | TODO |
| **LIB / HELPERS** ||||
| T08 | `src/lib/supabase.ts` — Supabase client init (browser + server) | Person A | TODO |
| T09 | `src/lib/replicate.ts` — helpers: trigger pixel art job (img2img + text-only fallback), poll job status, save result to Storage | Person A | TODO |
| T10 | `src/lib/claude.ts` — generate activity description: given pet profile + time of day + random seed, return `{ activity, scene }` | Person A | TODO |
| **API ROUTES** ||||
| T11 | `POST /api/pets` — create pet record with `session_id`, return `{ id }` | Person A | TODO |
| T12 | `GET /api/pets/[id]` — fetch pet profile, `pixel_art_url`, `current_activity`, `current_scene` | Person A | TODO |
...
| T14 | `GET /api/pets/[id]/pixel-art-status` — poll Replicate; on complete save image to Storage + update DB; on fail trigger retry/fallback logic | Person A | TODO |
| T15 | `GET /api/pets/[id]/activity` — call Claude to generate activity based on pet profile, time of day, random seed; save to `current_activity` + `current_scene`; return `{ activity, scene, generated_at }` | Person C | TODO |
| T16 | `POST /api/pets/[id]/claim` — verify `session_id`, set `user_id` on pet record | Person A | TODO |
| **FRONTEND — LANDING PAGE** ||||
| T17 | `src/app/page.tsx` — hero section, tagline ("A place where your pet lives on"), "Create my pet" CTA | Person B | TODO |
| T18 | `tailwind.config.ts` — add design tokens; implement Cosmic palette (`#1A1035` bg, `#F5C842` gold, `#7C5CBF` accent) as primary; keep Warm Amber tokens as alternative | Person B | TODO |
...
| T24 | Emotional loading state on `/pet/[id]`: soft gradient background, floating particle/star animation, species-based placeholder sprite with shimmer/pulse | Person B | TODO |
| T25 | Rotating warm messages (change every 3-4s), personalized with pet name: "正在为 [name] 搭建她的新家...", "[name] is exploring the meadow for the first time...", etc. | Person B | TODO |
| T26 | Poll `GET /api/pets/[id]/pixel-art-status` every 3s; minimum 5s display time even if generation is instant; add reassurance message after 30s ("Still working on it — good things take time") | Person B | TODO |
| T27 | Transition to ready state: loading messages fade out → scene fades in → first activity text appears ("*[name]* has arrived in their new world") | Person B | TODO |
| **FRONTEND — MAIN PET PAGE** ||||
| T28 | `src/app/pet/[id]/page.tsx` — ready state layout: navbar (Sign in), full-width scene image, pet name + species badge, activity text, scene details (time of day, weather, mood) | Person B | TODO |
| T29 | Scene image: fade-in transition from loading → real pixel art; fallback to species sprite if generation failed (silent, no error shown) | Person B | TODO |
| T30 | Activity display: call `GET /api/pets/[id]/activity` on page load; show narrative and scene context below the image | Person B | TODO |
| **AUTH** ||||
| T31 | Navbar "Sign in" button triggers Supabase Google OAuth flow | Person B | TODO |
| T32 | Auth callback: if localStorage has `session_id`, call `POST /api/pets/[id]/claim`, clear `session_id`, redirect to `/pet/[id]` | Person B | TODO |
| **ASSETS** ||||
| T33 | Add fallback pixel art sprites to `public/sprites/`: `dog.png`, `cat.png`, `bird.png`, `rabbit.png`, `other.png` | Person A | TODO |
...
| T38 | Mobile responsive check: test on iPhone SE (375px) and iPhone 14 (390px) viewport sizes | Person B | TODO |
